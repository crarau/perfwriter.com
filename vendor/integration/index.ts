  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('PerfWriter');

        const virtualModuleId = 'PerfWriter:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-PerfWriter-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`PerfWriter \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`PerfWriter config has been loaded.`);
        }
      },
